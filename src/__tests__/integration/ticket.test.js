...
describe("Ticket Model Test", () => {
  describe("Event ID Field", () => {
    it("Should throw an error if event id is not provided", () => {
      ...
      expect(validationErrors.errors["eventId"].message).toBe("Please provide Event Id");
    });
    it.only("Should throw an error if event id provided is not a mongodb object id type", () => {
      const ticket = new Ticket({ eventId: "1234" });
      const validationErrors = ticket.validateSync();
      expect(validationErrors.errors["eventId"]).toBeDefined();
    });
    it("Should pass if event id provided is  a mongodb object id type", () => {
      const ticket = new Ticket({ eventId: new mongoose.Types.ObjectId("6696475178b2ced7e1b87d40") });
      const validationErrors = ticket.validateSync();
  ...
  describe("User ID Field", () => {
    it("Should throw an error if user id is not provided", () => {
      ...
      expect(validationErrors.errors["userId"].message).toBe("Please provide User Id");
    });
    ...
    it("Should pass if user id provided is  a mongodb object id type", () => {
      const ticket = new Ticket({ userId: new mongoose.Types.ObjectId("6696475178b2ced7e1b87d40") });
      const validationErrors = ticket.validateSync();
  ...
  describe("Payment Field", () => {
    describe("Amount", () => {
      it("Should throw an error if amount is not provided", () => {
        ...
        expect(validationErrors.errors["payment.amount"].message).toBe("Please provide amount paid");
      });
    ...
    describe.only("Status", () => {
      it("Should throw an error if payment status is not provided", () => {
        ...
        expect(validationErrors.errors["payment.status"].message).toBe("Please provide payment id");
      });
